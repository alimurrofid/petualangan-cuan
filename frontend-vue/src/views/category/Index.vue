...
<template>
  ...
  <div class="flex-1 space-y-6 pt-2 text-foreground" v-else>
    ...
    <Tabs v-model:model-value="currentTab" class="space-y-6">
      ...
      <div
        class="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div v-for="item in filteredCategories" :key="item.id" @click="openEdit(item)"
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="min-w-0 flex-1 space-y-1">
              <p
                class="font-bold text-lg tracking-wide truncate leading-tight text-foreground group-hover:text-primary transition-colors">
                {{ item.name }}</p>
        ...
        <div v-if="filteredCategories.length === 0"
          class="col-span-full py-20 flex flex-col items-center justify-center text-muted-foreground border-2 border-dashed border-border rounded-3xl opacity-50 bg-muted/20">
        ...
        <DialogFooter class="p-6 border-t bg-muted/5 flex flex-row items-center justify-between gap-2">
          ...
          <div class="flex gap-2 ml-auto">
            ...
            <Button @click="handleSave" type="button"
              class="bg-gradient-to-r from-emerald-600 to-teal-500 text-white hover:from-emerald-500 hover:to-teal-400 px-6 shadow-md hover:bg-foreground/90"
      ...
      <DialogContent
        ...
        <Tabs default-value="icons" class="flex-1 flex flex-col overflow-hidden">
          ...
          <TabsContent value="icons" class="flex-1 overflow-y-auto p-6 mt-0">
            ...
            <p v-if="filteredCategoryIcons.length === 0" class="text-center text-sm text-muted-foreground py-8">Tidak
              ada
              icon cocok.</p>
          </TabsContent>
