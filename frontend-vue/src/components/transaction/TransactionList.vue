...
<template>
     <div class="h-full flex flex-col">
        <div v-if="transactionStore.isLoading && transactionStore.transactions.length === 0" class="flex-1 flex items-center justify-center p-8">
            <span class="loading loading-spinner loading-md text-muted-foreground"></span>
        </div>

        <div v-else-if="transactionStore.transactions.length === 0" class="flex-1 flex flex-col items-center justify-center text-muted-foreground opacity-60 text-center p-8">
        ...
        <div v-else class="flex-1 overflow-y-auto space-y-6 pr-2 custom-scrollbar p-1">
            <div v-for="group in groupedTransactions" :key="group.date" class="space-y-3">
                ...
                <div v-for="t in group.items" :key="t.id" class="group relative flex items-center justify-between p-3 rounded-2xl hover:bg-muted/50 transition-all cursor-pointer border border-transparent hover:border-border">
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                <a v-if="t.attachment" :href="`/api/${t.attachment.replace(/^\//, '')}`" target="_blank"
                                    @click.stop class="text-xs text-blue-500 hover:text-blue-700 flex items-center">
            ...
            <div class="flex justify-center gap-4 py-4 mt-4 border-t border-border">
                <Button 
                    variant="outline" 
                    size="sm" 
                    :disabled="transactionStore.paginationMeta.page <= 1"
                    @click="$emit('page-change', transactionStore.paginationMeta.page - 1)"
                >
                    Previous
                </Button>
                <div class="flex items-center text-xs text-muted-foreground">
                    Page {{ transactionStore.paginationMeta.page }} / {{ Math.ceil(transactionStore.paginationMeta.total / transactionStore.paginationMeta.limit) || 1 }}
                </div>
                <Button 
                    variant="outline" 
                    size="sm" 
